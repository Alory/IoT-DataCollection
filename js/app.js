...
(function(){
	...
	{
		console.log(Date.now() + "-data is: " + JSON.stringify(data) + "\n");
	}
	...
	{	
		...
		//my edit
		iotsensor
			.accelerometerCallback(showData)
			.gyroscopeCallback(showData)
			.magnetometerCallback(showData)
			.barometerCallback(showData)
			.temperatureCallback(showData)
			.humidityCallback(showData)
			.sflCallback(showData)
			.errorCallback(connectionError);
	
		setEventHandlers();	
		initSensorFunctions();
		initSensorButtons();
		console.log(JSON.stringify(app.sensors));
	}	
		...
		iotsensor.startScanningForDevices(
			function(device)
			{
				...
				if(device.name === evothings.iotsensor.SFL || device.name === evothings.iotsensor.RAW)
				{
					...
					console.log("test: " + device.address);//my edit
				}
			}
		);
	...
	/* WangChao 2017.12.28 */
	app.connectMultiDevices = function()
	{
		console.log("start connect multiple deviece!")
				...
				iotsensor.connectToDevice(
					...
					function()
					{
						...
						getSettings(
							function()
							{
								console.log("SENSOR_COMBINATION: " + app.settings.CURRENT.SENSOR_COMBINATION);
							}
						);
						for(sensorId in app.sensors){
							console.log("app.sensors:");
							console.log(JSON.stringify(app.sensors));
							console.log("sensorId: "+sensorId);
							app.sensors[sensorId].active = 'true';
							console.log("app.sensors active: "+app.sensors[sensorId].active);
			...
			iotsensor.connectToDevice(
				...
				function()
				{
					...
					getSettings(
						function()
						{
							showAvailableSensors();//显示可用的sensors
							updateSettingsView();
